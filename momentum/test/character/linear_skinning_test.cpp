 ...
 protected:
  void SetUp() override {
    ...
    for (size_t i = 0; i < inverseBindPose.size(); ++i) {
      inverseBindPose[i] = Eigen::Affine3f::Identity();
    }
  ...
  void setupSkinWeights() {
    ...
    for (int j = 1; j < kMaxSkinJoints; ++j) {
      skin.index(0, j) = 0;
      skin.weight(0, j) = 0.0f;
    }
    ...
    for (int j = 2; j < kMaxSkinJoints; ++j) {
      skin.index(1, j) = 0;
      skin.weight(1, j) = 0.0f;
    }
...
TEST_F(LinearSkinningTest, ApplySSDMeshErrors) {
  ...
  EXPECT_DEATH(applySSD(inverseBindPose, skin, mesh, skeletonState, smallOutputMesh), ".*");

  // Create output mesh with different number of normals
  MeshT<float> badNormalsMesh = mesh;
  badNormalsMesh.normals.pop_back();

  EXPECT_DEATH(applySSD(inverseBindPose, skin, mesh, skeletonState, badNormalsMesh), ".*");

  // Create output mesh with different number of faces
  MeshT<float> badFacesMesh = mesh;
  badFacesMesh.faces.push_back(Vector3i(0, 1, 2));

  EXPECT_DEATH(applySSD(inverseBindPose, skin, mesh, skeletonState, badFacesMesh), ".*");
}
...
TEST_F(LinearSkinningTest, GetInverseSSDTransformationErrors) {
  ...
  EXPECT_DEATH(getInverseSSDTransformation(inverseBindPose, skin, smallState, 0), ".*");

  // Test with invalid vertex index
  EXPECT_DEATH(getInverseSSDTransformation(inverseBindPose, skin, skeletonState, 99), ".*");
}
