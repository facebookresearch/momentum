...
TEST_F(CollisionGeometryStateTest, UpdateWithTransformedJoints) {
  ...
  skeletonState.jointState[0].transformation.rotate(
      Eigen::AngleAxisf(M_PI / 2, Vector3f::UnitY())); // 90 degrees around Y
...
TEST_F(CollisionGeometryStateTest, OverlapsNonOverlapping) {
  ...
  EXPECT_FALSE(result);
  // The closest points should be at the origins
  EXPECT_NEAR(outClosestPoints[0], 0.0f, 1e-5f);
  EXPECT_NEAR(outClosestPoints[1], 0.0f, 1e-5f);
}
