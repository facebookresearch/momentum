...
// ---------------------------------------------------------------------------
// VertexPositionConstraintErrorFunctionT — FuncDim=3
// ---------------------------------------------------------------------------
TYPED_TEST(Momentum_ErrorFunctionsTest, VertexPositionConstraint_GradientsAndJacobians) {
  ...
  VertexPositionConstraintErrorFunctionT<T> errorFunction(character, pt);
  const size_t numVerts = character.mesh->vertices.size();
  for (size_t i = 0; i < 5; ++i) {
    errorFunction.addConstraint(
        VertexPositionConstraintDataT<T>(i % numVerts, uniform<Vector3<T>>(0, 1), 1.0f));
  }
...
TYPED_TEST(
  ...
  VertexPlaneConstraintErrorFunctionT<T> errorFunction(character, pt);
  const size_t numVerts = character.mesh->vertices.size();
  for (size_t i = 0; i < 5; ++i) {
    VertexPlaneConstraintDataT<T> data;
...
// ---------------------------------------------------------------------------
// VertexPlaneConstraintErrorFunctionT — FuncDim=1
// ---------------------------------------------------------------------------
...
TYPED_TEST(Momentum_ErrorFunctionsTest, CrossVal_Position_NoBS) {
  ...
  VertexPositionConstraintErrorFunctionT<T> newFunc(character, pt);
  ...
  for (size_t i = 0; i < 5; ++i) {
    ...
    newFunc.addConstraint(VertexPositionConstraintDataT<T>(vIdx, target, 1.0f));
  }
...
TYPED_TEST(Momentum_ErrorFunctionsTest, VertexNormalConstraint_Normal_GradientsAndJacobians) {
  ...
  VertexNormalConstraintErrorFunctionT<T> errorFunction(character, pt, T(1), T(0));
  const size_t numVerts = character.mesh->vertices.size();
  for (size_t i = 0; i < 5; ++i) {
    size_t vIdx = i % numVerts;
    ...
    errorFunction.addConstraint(
        VertexNormalConstraintDataT<T>(vIdx, targetPosition, targetNormal, 1.0f));
  }
...
TYPED_TEST(
  ...
  VertexNormalConstraintErrorFunctionT<T> errorFunction(character, pt, T(0.5), T(0.5));
  const size_t numVerts = character.mesh->vertices.size();
  for (size_t i = 0; i < 5; ++i) {
    size_t vIdx = i % numVerts;
    auto targetPosition = uniform<Vector3<T>>(0, 1);
...
TYPED_TEST(Momentum_ErrorFunctionsTest, CrossVal_Normal_NoBS) {
  ...
  VertexNormalConstraintErrorFunctionT<T> newFunc(character, pt, T(1), T(0));
  ...
  for (size_t i = 0; i < 5; ++i) {
    ...
    newFunc.addConstraint(VertexNormalConstraintDataT<T>(vIdx, targetPosition, targetNormal, 1.0f));
  }
...
TYPED_TEST(Momentum_ErrorFunctionsTest, VertexPositionConstraint_PerConstraintWeight) {
  ...
  errorFunction.addConstraint(
      VertexPositionConstraintDataT<T>(0, uniform<Vector3<T>>(10, 20), 0.0f));
  // Add a normal-weight constraint
  errorFunction.addConstraint(
      VertexPositionConstraintDataT<T>(1 % numVerts, uniform<Vector3<T>>(0, 1), 1.0f));
  // Add a double-weight constraint
  errorFunction.addConstraint(
      VertexPositionConstraintDataT<T>(2 % numVerts, uniform<Vector3<T>>(0, 1), 2.0f));
...
TYPED_TEST(Momentum_ErrorFunctionsTest, VertexPositionConstraint_GetErrorOnly) {
  ...
  VertexPositionConstraintErrorFunctionT<T> errorFunction(character, pt);
  errorFunction.addConstraint(VertexPositionConstraintDataT<T>(0, uniform<Vector3<T>>(0, 1), 1.0f));
...
TYPED_TEST(
  ...
  VertexPositionConstraintErrorFunctionT<T> errorFunction(character, pt, T(2), T(1));
  for (size_t i = 0; i < 5; ++i) {
    errorFunction.addConstraint(
        VertexPositionConstraintDataT<T>(i % numVerts, uniform<Vector3<T>>(0, 1), 1.0f));
  }
...
TYPED_TEST(Momentum_ErrorFunctionsTest, CrossVal_Plane_NoBS) {
  ...
  VertexPlaneConstraintErrorFunctionT<T> newFunc(character, pt);
...
// VertexNormalConstraintErrorFunctionT.
// ---------------------------------------------------------------------------
TEST(VertexNormalDebug, InvestigateJacobianMismatch) {
  ...
  VertexNormalConstraintErrorFunctionT<T> newFunc(character, pt, 1.0f, 0.0f);
...
TEST(VertexNormalDebug, AccurateModeDebug) {
  ...
  VertexNormalConstraintErrorFunctionT<T> newFunc(
  ...
  newFunc.addConstraint(
      VertexNormalConstraintDataT<T>(
          vertexIndex, targetPosition, Eigen::Vector3<T>::UnitZ(), 1.0f));
  ...
  VertexNormalConstraintErrorFunctionT<T> posOnlyFunc(character, pt, 0.0f, 1.0f);
  posOnlyFunc.setWeight(1.0f);
  posOnlyFunc.addConstraint(
      VertexNormalConstraintDataT<T>(vertexIndex, targetPosition, sourceNormal.normalized(), 1.0f));
