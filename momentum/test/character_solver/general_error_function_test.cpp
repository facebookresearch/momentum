...
// Tests for MultiSourceErrorFunctionT edge cases:
...
// Test getError path for MultiSourceErrorFunctionT (using CenterOfMass as concrete implementation)
TEST(MultiSourceErrorFunctionTest, GetErrorPath) {
...
TEST(MultiSourceErrorFunctionTest, ZeroGlobalWeight) {
  const Character character = createTestCharacter(8);
  const auto& skel = character.skeleton;
  const auto& pt = character.parameterTransform;
  const auto numParams = pt.numAllModelParameters();
...
// Test needsMesh() returns false for MultiSourceErrorFunctionT with JointPoint contributions
TEST(MultiSourceErrorFunctionTest, NeedsMeshReturnsFalseForJointOnly) {
  const Character character = createTestCharacter(8);
  const auto& skel = character.skeleton;
  const auto& pt = character.parameterTransform;
  ...
  // CoM uses only JointPoint contributions, no Vertex
  CenterOfMassConstraintT<float> constraint;
...
TEST(MultiSourceErrorFunctionTest, JacobianSize) {
  const Character character = createTestCharacter(8);
  const auto& skel = character.skeleton;
  const auto& pt = character.parameterTransform;
  const auto numParams = pt.numAllModelParameters();
