...
TYPED_TEST(Momentum_ErrorFunctionsTest, FixedAxisDiffErrorL2_JacobianWithZeroFunctionValue) {
  using T = typename TestFixture::Type;
  ...
  // Set the global axis to match the rotated local axis, so f = 0
  const Vector3<T> localAxis = Vector3<T>::UnitY();
  const Vector3<T> rotatedAxis =
      (state.jointState[2].transform.toRotationMatrix() * localAxis).normalized();

  FixedAxisDiffErrorFunctionT<T> errorFunction(skeleton, character.parameterTransform);
  const T kTestWeight = 1.0;
  std::vector<FixedAxisDataT<T>> cl{FixedAxisDataT<T>(localAxis, rotatedAxis, 2, kTestWeight)};
  errorFunction.setConstraints(cl);
