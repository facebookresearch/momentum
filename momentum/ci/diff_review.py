...
def check_diff_summary_oss(summary):
    ...
    internal_url_patterns = [
        (r"internalfb\.com", "internalfb.com URL"),
        (r"our\.intern\.facebook\.com", "intern.facebook.com URL"),
        (r"fb\.workplace\.com", "workplace URL"),
        (r"bunnylol\b", "bunnylol reference"),
    ]
    for pattern, name in internal_url_patterns:
        if re.search(pattern, public_portion):
            issues.append(
                f"  {name} in public portion of diff summary. "
                "Move behind 'Internal:' line."
            )

    # Check for task/SEV/diff references in public portion
    ref_patterns = [
        (r"\bT\d{6,}\b", "Task reference (T...)"),
        (r"\bS\d{5,}\b", "SEV reference (S...)"),
        (r"\bD\d{6,}\b", "Diff reference (D...)"),
    ]
    ...
    # Check for references to internal tools and paths
    internal_ref_patterns = [
        (r"\bbuck\s+(?:build|test|run|query|targets)\b", "buck command reference"),
        (r"\barc\s+(?:lint|diff|land)\b", "arc command reference"),
        (r"\b(?:fbsource|fbcode)\b", "internal repo path reference"),
        (r"\bConfigerator\b", "Configerator reference"),
        (r"\bSandcastle\b", "Sandcastle reference"),
        (r"\bCitadel\b", "Citadel reference"),
        (r"\bSkycastle\b", "Skycastle reference"),
        (r"\bDevmate\b", "Devmate reference"),
        (r"\.llms/", ".llms/ path reference"),
        (r"\bci_(?:shell_test|skycastle|sandcastle)\b", "internal CI macro reference"),
        (r"\barvr/", "internal file path reference"),
    ]
    for pattern, name in internal_ref_patterns:
        if re.search(pattern, public_portion, re.IGNORECASE):
            issues.append(
                f"  {name} in public portion of diff summary. "
                "Move behind 'Internal:' line."
            )

    # Check the title specifically — it becomes the GitHub commit title
    # and is highly visible. Run all the same patterns against the title
    # with title-specific messages.
    if title:
        all_patterns = (
            internal_url_patterns
            + ref_patterns
            + [(p, n) for p, n in internal_ref_patterns]
        )
