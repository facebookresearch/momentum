...
///
/// VertexConstraintErrorFunctionT handles constraints involving a single mesh vertex.
...
class VertexConstraintErrorFunctionT : public SkeletonErrorFunctionT<T> {
 public:
  ...
  explicit VertexConstraintErrorFunctionT(
      const Character& character,
      const ParameterTransform& parameterTransform,
      const T& lossAlpha = GeneralizedLossT<T>::kL2,
      const T& lossC = T(1));

  ~VertexConstraintErrorFunctionT() override = default;
