...
void SkeletonDerivativeT<T>::accumulateJointJacobian(
  ...
  // ---- Combined fast path for 1 JointPoint + N JointDirections ----
  // Mirror of the gradient's combined path. For FuncDim > 1, the scalar triple-product
  // identity doesn't apply directly. Instead, precompute a combined FuncDim×3 matrix:
  //   M_dirs = Σ_all scale * dfdv[i] * skew(worldVec[i])
  // At each ancestor joint j, adjust for the point's per-joint offset:
  //   M_rot = M_dirs - scaledDfdvPoint * skew(t_j)
  //   jc[d] = M_rot * rotAxis[d]
  // This reduces per-joint rotation cost from numVecs matmuls to 1 precompute + 3 matmuls.
  {
    size_t pointIdx = SIZE_MAX;
    bool canUseCombined = (numVecs > 1);
    if (canUseCombined) {
    ...
    if (canUseCombined) {
      ...
      // Precompute scaled dfdv for the point (reused for translation and scale DOFs)
      const Eigen::Matrix<T, FuncDim, 3> scaledDfdvPoint = scale * dfdvs[pointIdx];

      // Helper: compute M * skew(v), i.e. the FuncDim×3 matrix such that
      // (M * skew(v)) * a = M * (v × a) for any vector a.
      auto matTimesSkew = [](const Eigen::Matrix<T, FuncDim, 3>& M,
                             const Eigen::Vector3<T>& v) -> Eigen::Matrix<T, FuncDim, 3> {
        Eigen::Matrix<T, FuncDim, 3> result;
        result.col(0) = M.col(1) * v[2] - M.col(2) * v[1];
        result.col(1) = M.col(2) * v[0] - M.col(0) * v[2];
        result.col(2) = M.col(0) * v[1] - M.col(1) * v[0];
        return result;
      };

      // M_dirs_neg = -(Σ_all scale * dfdv[i] * skew(worldVec[i]))
      // The negation comes from the identity: dfdv * (rotAxis × offset)
      //   = -(dfdv * skew(offset)) * rotAxis, since rotAxis × offset = -skew(offset) * rotAxis.
      // By precomputing M_dirs_neg negated, the per-joint formula becomes:
      //   M_rot = M_dirs_neg + matTimesSkew(scaledDfdvPoint, t_j)
      //         = -(Σ scale*dfdv[i]*skew(offset_i))
      //   jc = M_rot * rotAxis[d]
      Eigen::Matrix<T, FuncDim, 3> M_dirs_neg = -matTimesSkew(scaledDfdvPoint, worldVecs[pointIdx]);
      for (size_t i = 0; i < numVecs; ++i) {
        if (i == pointIdx || dfdvs[i].isZero()) {
          continue;
        }
        M_dirs_neg.noalias() -= matTimesSkew(scale * dfdvs[i], worldVecs[i]);
      }
      ...
      while (jntIndex != kInvalidIndex) {
        ...
        for (size_t d = 0; d < 3; ++d) {
          ...
          const Eigen::Matrix<T, FuncDim, 1> jc =
              scaledDfdvPoint * jntState.getTranslationDerivative(d);
          for (auto index = parameterTransform_.transform.outerIndexPtr()[jntParamIndex + d];
               index < parameterTransform_.transform.outerIndexPtr()[jntParamIndex + d + 1];
               ++index) {
            if (enabledParameters_.test(parameterTransform_.transform.innerIndexPtr()[index])) {
        ...
        // Rotation DOFs: use combined matrix
        // M_rot = M_dirs_neg + matTimesSkew(scaledDfdvPoint, t_j)
        //       = -(Σ scale*dfdv[i]*skew(offset_i))
        // where offset_point = worldVec_point - t_j, offset_dir_k = worldVec_k.
        const Eigen::Matrix<T, FuncDim, 3> M_rot =
            M_dirs_neg + matTimesSkew(scaledDfdvPoint, jntState.translation());
        for (size_t d = 0; d < 3; ++d) {
          if (!activeJointParams_[jntParamIndex + 3 + d]) {
            continue;
          }
          const Eigen::Matrix<T, FuncDim, 1> jc = M_rot * jntState.rotationAxis.col(d);
          for (auto index = parameterTransform_.transform.outerIndexPtr()[jntParamIndex + 3 + d];
               index < parameterTransform_.transform.outerIndexPtr()[jntParamIndex + 3 + d + 1];
               ++index) {
            if (enabledParameters_.test(parameterTransform_.transform.innerIndexPtr()[index])) {
        ...
        if (activeJointParams_[jntParamIndex + 6]) {
          const Eigen::Vector3<T> offset = worldVecs[pointIdx] - jntState.translation();
          const Eigen::Matrix<T, FuncDim, 1> jc =
              scaledDfdvPoint * jntState.getScaleDerivative(offset);
          for (auto index = parameterTransform_.transform.outerIndexPtr()[jntParamIndex + 6];
               index < parameterTransform_.transform.outerIndexPtr()[jntParamIndex + 6 + 1];
               ++index) {
            if (enabledParameters_.test(parameterTransform_.transform.innerIndexPtr()[index])) {
