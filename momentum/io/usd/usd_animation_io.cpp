...
void saveSkeletonStatesToUsd(
  ...
  for (size_t frame = 0; frame < skeletonStates.size(); ++frame) {
    ...
    animPrim.GetTranslationsAttr().Set(translations, timeCode);
    animPrim.GetRotationsAttr().Set(rotations, timeCode);
    animPrim.GetScalesAttr().Set(scales, timeCode);
  }
}
...
std::tuple<std::vector<SkeletonState>, std::vector<float>> loadSkeletonStatesFromUsd(
  ...
  for (const auto& prim : stage->Traverse()) {
    ...
    for (const double sample : timeSamples) {
      ...
      animPrim.GetTranslationsAttr().Get(&translations, timeCode);
      animPrim.GetRotationsAttr().Get(&rotations, timeCode);
      animPrim.GetScalesAttr().Get(&scales, timeCode);
      ...
      for (size_t j = 0; j < numJoints; ++j) {
        ...
        if (j < scales.size()) {
          jointParams[base + 6] = std::log2(static_cast<float>(scales[j][0]));
        }
      }

      states.emplace_back(jointParams, skeleton, false);
