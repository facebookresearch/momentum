...
void solve_ik()
{
  ...
  SkeletonSolverFunctionT<T> solverFunction(&skeleton, &transform);
   ...
   auto errorFunction = std::make_shared<FullyDifferentiablePositionErrorFunction>(skeleton, transform);

  //  // add to solvable
   
  //  auto errorfunction2 = std::make_shared<PositionErrorFunction>(skeleton, transform);
  ...
  std::vector<PositionConstraintT<T>> constraints;
  constraints.push_back(
  PositionConstraintT<T>(Vector3<T>::Zero(), Vector3<T>(30.0, 0.0, 40.0), 6, 1.0));

  errorFunction->addConstraint(PositionConstraintT<T>(Vector3<T>::Zero(), Vector3<T>(30.0, 0.0, 40.0), 6, 1.0));
  errorFunction->setWeight(1);
  ...
  //float __vector(8) Eigen::internal::pload<float __vector(8)>(Eigen::internal::unpacket_traits<float __vector(8)>::type const*) ()
