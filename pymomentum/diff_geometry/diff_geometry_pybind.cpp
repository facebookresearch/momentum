...
void defineForwardKinematics(py::module_& m) {
...
void defineUtilityFunctions(py::module_& m) {
  m.def(
      "find_closest_points",
      &findClosestPoints,
...
PYBIND11_MODULE(diff_geometry, m) {
  ...
  defineForwardKinematics(m);

  // Define character operations
  defineCharacterOperations(m);

  // Define parameter transform operations
  defineParameterTransforms(m);

  // Define blend shape operations
  defineBlendShapeOperations(m);

  // Define utility functions
  defineUtilityFunctions(m);
}
