...
class TestDiffGeometry(unittest.TestCase):
    ...
    def test_compute_vertex_normals(self) -> None:
        ...
        character = pym_models.load_default_blueman()
        n_vertices = character.mesh.n_vertices
        vertex_positions = torch.from_numpy(character.mesh.vertices)
        triangles = torch.from_numpy(character.mesh.faces)
        # test no batch dimension
        vertex_normals = pym_diff_geometry.compute_vertex_normals(
            vertex_positions, triangles
        )
        self.assertEqual(vertex_normals.shape, (n_vertices, 3))

        # test with batch dimension on vertex_positions
        vertex_positions = vertex_positions.unsqueeze(0).expand(2, -1, -1)
        vertex_normals = pym_diff_geometry.compute_vertex_normals(
            vertex_positions, triangles
        )
        self.assertEqual(vertex_normals.shape, (2, n_vertices, 3))
