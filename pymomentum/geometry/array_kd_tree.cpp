...
void findClosestPointsOnMeshImpl(
    ...
    const VectorArrayAccessor<int32_t, 3>& tgtFacesAcc,
    const std::vector<py::ssize_t>& srcIndices,
    const std::vector<py::ssize_t>& tgtIndices,
    typename VectorArrayAccessor<T, 3>::ElementView resultPoints,
  ...
  // Get target mesh data as matrices
  const auto targetVerticesMat = tgtVerticesAcc.toMatrix(tgtIndices);
  const auto targetFacesMat = tgtFacesAcc.toMatrix(tgtIndices);

  const int64_t nTgtVertices = targetVerticesMat.rows();

  MT_THROW_IF(
      targetFacesMat.cols() != 3 || targetVerticesMat.cols() != 3,
  ...
  auto faceBounds = facesView.bounds();
  if (faceBounds.has_value()) {
    const auto [minIdx, maxIdx] = *faceBounds;
    MT_THROW_IF(
        maxIdx >= nTgtVertices,
  ...
  // Build BVH from mesh
  const TriBvh targetTree(Eigen::MatrixX3<T>{targetVerticesMat}, Eigen::MatrixX3i{targetFacesMat});
...
std::tuple<py::array, py::array, py::array, py::array> findClosestPointsOnMeshArray(
  ...
  // Verify that faces are int32
  MT_THROW_IF(
      facesInfo.format != py::format_descriptor<int32_t>::format(),
      "find_closest_points_on_mesh: faces_target must be int32, got {}",
      facesInfo.format);
  ...
  // Validate faces_target: shape [..., nTgtFaces, 3]
  checker.validateBuffer(
      faces_target, "faces_target", {-3, 3}, {"nTgtFaces", "xyz"}, false /* allowEmpty */);
  ...
  if (checker.isFloat64()) {
    ...
    VectorArrayAccessor<int32_t, 3> facesAcc(facesArr, leadingDims, nTgtFaces);
